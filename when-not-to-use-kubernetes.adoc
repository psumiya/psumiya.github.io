...
Due to competing priorities we somehow fell behind officially supported version of Kubernetes. We had to spend several cycles with multiple engineers learning about the cluster setup and safely perform the upgrade.
...
In my opinion, Kubernetes and all of its complexity should really be left to the _very_ few teams who actually do need it solve some problem that is very special to their domain. For those use cases, Kubernetes would indeed be providing a business value, and that would be incentive for the business at large to build and maintain teams with Kubernetes expertise.
